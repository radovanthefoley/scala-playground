...
class KMeans {
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    if (oldMeans.size != newMeans.size) new IllegalArgumentException("sequencies not equal in size")
    val iter = oldMeans.iterator
    newMeans.forall(_.squareDistance(iter.next()) <= eta)
  }
